...
"""
Archive Explorer v5
Fusion complète — interface simple et intuitive pour novices
...
class EditorTab(QWidget):
    def __init__(self, parent=None):
        ...
        # Barre de boutons (en bas) — organisée en groupes logiques
        bbar = QHBoxLayout()
...
class FileTree(QTreeWidget):
    ...
    def _load_arc_children(self, parent, arc_path):
        try:
            ...
            # Construction arbre de sous-dossiers
            tree = {}
            for f in files:
                parts = Path(f).parts
                cur   = tree
                for i, part in enumerate(parts):
                    ...
                    if part not in cur:
                        cur[part] = {'_path': f if is_last else None, '_children': {}}
                    elif is_last:
                        cur[part]['_path'] = f
            ...
            def fill(par, d):
                for name, info in sorted(d.items()):
                    if info['_path'] is not None:
                        ext  = Path(name).suffix.lower()
                        icon = self.EXT_ICON.get(ext, '📄')
                        ch   = QTreeWidgetItem(par, [f"{icon} {name}", ""])
                        ch.setData(0, Qt.UserRole, ('arc_file', arc_path, info['_path']))
                        ch.setToolTip(0, info['_path'])
                    else:
                        fi = QTreeWidgetItem(par, [f"📁 {name}", ""])
                        fi.setData(0, Qt.UserRole, ('dir_placeholder',))
                        fill(fi, info['_children'])
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Archive Explorer v5")
